...
if (isset($_GET['delete'])) {
    $emp_id = $_GET['delete'];
    mysqli_query($conn, "DELETE FROM employee WHERE emp_id='$emp_id'");
...
if (isset($_GET['update'])) {
    $emp_id = $_GET['update'];
    $res = mysqli_query($conn, "SELECT * FROM employee WHERE emp_id='$emp_id'");
...
if (isset($_POST['update_submit'])) {
    ...
    mysqli_query($conn, "UPDATE employee SET firstname='$firstname', lastname='$lastname', role='$role' WHERE emp_id='$emp_id'");
    header("Location: display.php");
    exit;
}
?>
...
<table>
    ...
    <?php
    $sql = mysqli_query($conn, "SELECT * FROM employee ORDER BY emp_id ASC");
    while ($r = mysqli_fetch_assoc($sql)) {
        echo "<tr>
            ...
            <td>{$r['lastname']}</td>
            <td>{$r['role']}</td>
            <td><a href='?delete={$r['emp_id']}' onclick=\"return confirm('Are you sure?')\">Delete</a></td>
